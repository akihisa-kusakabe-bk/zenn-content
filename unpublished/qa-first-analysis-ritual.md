...
正直、自分も最初はそうでした。品質戦略の担当になったとき、「分析しよう」と意気込んでパレート分析やODCの教科書を開いた。でもデータが整っていない。ツールもない。チームに「分析会議やりましょう」と言っても「その時間あるならテスト回したい」と返される。結局、最初の3ヶ月は何も始められなかった。
...
自分のチームでも最初は「月1回の品質分析会議」を設定した。2時間の枠を取って、バグデータを集めて、スライドを作って……2回目で誰も資料を準備しなくなり、3回目で自然消滅した。振り返る頃にはバグの記憶が薄れていて、「たしかこういう感じだった気がする」という曖昧な議論にしかならなかった。

週15分に変えたら、続いた。品質分析会議を月1回2時間やるより、週15分を4回やる方がはるかに効果があります。
...
まず自分一人で、今週のバグを15分振り返る。2〜3週間続けてパターンが見えてきたら、「こういうの見えてきたんですけど」と共有する。

自分の場合、3週間分の記録を見せたとき、開発リーダーが「これ、決済まわりだけ毎週出てるじゃん」と気づいてくれた。そこから設計レビューに呼ばれるようになった。実績を見せた方が、チームは動きます。
...
- **Critical** = サービスが止まる、データが消えるなど、最も深刻なレベル。「テスト中に決済が二重課金される」のようなバグ
...
- **DDP（Defect Detection Percentage）** = テストで見つけたバグ数 ÷ 全バグ数。テストがどれだけバグを捕まえられたかの割合。高いほどテストが有効
