...
前回の記事で「テスト＝小テスト、QA＝カリキュラム設計」とたとえた。この続きで考えてみる。

テスト実行だけのQAは、**小テストを繰り返すだけで成績を上げようとしている**ようなものだ。

小テストは大事だ。でも、本当に成績を上げたいなら、カリキュラム設計、教材選び、理解度チェック、成績分析、勉強法の見直しが必要になる。テストの「前」と「後」にやることがたくさんある。

QAも同じだ。テスト実行は小テスト。それ以外の活動が、カリキュラムや教材選びや勉強法の改善にあたる。
...
| 時間軸 | いつやるか | たとえ（勉強） |
|---|---|---|
| **上流** | プロダクトを作る前 | カリキュラム設計・教材選び（予防） |
| **中流** | 作りながら | 小テスト・理解度チェック（モニタリング） |
| **後工程＋横断** | 出した後＋常時 | 成績分析・勉強法の見直し（改善） |

![QA活動の全体マップとフィードバックループ](/images/qa-activities-beyond-testing/qa-activities-map.png)

実線は成果物の流れ、赤い点線がフィードバックだ。**後工程の学びが上流に戻るフィードバックループ**が回って初めて「仕組み」になる。

バグ分析で「仕様が曖昧だったから作り込まれた」とわかれば、次の要件レビューの精度が上がる。不具合傾向を蓄積すれば、観点カタログとしてテスト設計に活かせるし、「どこにバグが集中しているか」のデータとして変更リスク評価にも効く。このループが上手く回ると、同じバグが再発しにくくなる。
...
| **CI/CDへの品質指標組み込み** | ソースコード・テスト結果 | カバレッジ、静的解析結果 | → メトリクス |
...
ここが一番コスパがいい。**バグは、作り込まれた後に見つけるより、作り込まれる前に防ぐほうが、ずっと手戻りコストを抑えられる**。
...
:::details CI/CDとは？
CI（Continuous Integration）は、コードの変更を頻繁に統合して自動テストを回すこと。CD（Continuous Delivery/Deployment）は、テスト済みのコードを自動でデプロイすること。ここでは「コードを書いたら自動でチェックしてくれる仕組み」くらいの理解で十分だ。
:::
...
## 後工程＋横断：「出した後」と「ずっと」やること

ここからが「QAらしい」仕事だと自分は思っている。テストで見つけたバグを、**次は作り込まれないようにする**。この「戻すループ」があるかないかで、チームの品質水準は半年後にまるで変わる。
...
:::details メトリクスの注意点
数値は判断材料であって、目標ではない。バグ密度を下げることを目標にすると、バグを報告しなくなるだけだ。メトリクスは「現状を知る」ために使い、「目標を追う」ために使わないのがコツだ。
:::
...
バグ1件から改善施策が1つ出る。これを回すと、テスト（＝測る）の結果がQA（＝仕組みをつくる）に変わる。
...
| 中流 | 品質ゲート、CI/CD品質指標の組み込み、テスト設計、探索的テスト | 問題を早く見つける |
| 後工程＋横断 | メトリクス、バグ分析、不具合傾向の蓄積、プロセス改善 | 同じ失敗を繰り返さない |

最初に書いた勉強のたとえに戻ると、小テストだけで成績は上がらない。カリキュラムも教材選びも成績分析も全部ひっくるめて「学び」であるように、テスト実行だけが「QA」じゃない。
