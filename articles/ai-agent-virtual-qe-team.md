---
title: "AIエージェント10体で「仮想QAチーム」を組んだ話"
emoji: "🤖"
type: "tech"
...
:::message
**この記事でつかうもの**
- **Claude Code**：Anthropic社が提供するターミナルうえで動くAIコーディングアシスタント。チャット形式でファイル操作・コード生成・分析などができる
- **カスタムエージェント**：Claude Codeに「専門家の人格」を与える仕組み。Markdownファイル1つで、名前・得意分野・使えるツール・AIモデルを定義できる
- この記事では、このカスタムエージェントを10体作って「仮想チーム」にした話をする
:::

## QAチームが1人しかいない問題

自分はビットキーでソフトウェア品質戦略を担当している。QAに関わるメンバーは他にもいるが、品質戦略の専門職としては自分1人。品質データの分析、Qase[^qase]の運用設計、ダッシュボードの構築、ベンダー管理、レポート作成。数えたら10種類の専門業務があった。全部1人でやっている。

人を増やせればいいが、品質戦略の専門人材はそう簡単に採れない。

で、考えた。**AIエージェントに専門職を割り当てて「仮想QAチーム」を作れないか？**

きっかけはチューリング社・阿部氏のアプローチ（[参考ポスト](https://x.com/abehiraku/status/2033351793120534952)）。41ファイルの仮想PRチームを構築して開発プロセスを回している事例を見て、「QA版をやれるんじゃないか」と思った。

## なぜ「1体の万能AI」ではダメだったか

最初は1つのプロンプトに全部詰め込んだ。すぐに限界が見えた。

- **コンテキストが溢れる。** 品質データ分析の文脈でテスト設計の指示が混ざると、どちらも中途半端になる

:::details コンテキストとは？
AIが一度の会話で覚えていられる情報量のこと。人間でいう「ワーキングメモリ」に近い。会話が長くなったり、無関係な情報が混ざると、AIは古い情報を忘れたり、指示を取り違えたりする。専門分野を分けることで、1体あたりのコンテキストを小さく保てる。
:::
- **モデルの使い分けができない。** テスト戦略の判断はOpus[^opus-sonnet]が必要だが、データ集計はSonnetで十分。コストが無駄になる

**実際に起きた失敗：** 汎用AIにテスト管理ツールのテストケースを一括更新させたとき、`custom_fields`（複数形）を使い続けた。APIはstatus:trueを返すのにデータが反映されない。正解は`custom_field`（単数形）。73プロジェクト/217,948件の処理でこのミスは致命的だった。

専門特化したエージェントなら、そのAPIの癖をシステムプロンプトに書いておける。汎用AIでは無理だった。
...
Claude Codeのカスタムエージェント機能[^custom-agent]を使っている。セットアップは3ステップ。
...
# 2. 定義ファイルを置く
vim ~/.claude/agents/quality-analyst.md

# 3. Claude Codeから呼び出す（チャットで入力するだけ）
# > @quality-analyst このCSVを分析して
```

定義ファイルの中身はMarkdownだけ。
...
| **Read** | ファイルの中身を読む |
| **Write** | 新しいファイルを作成する |
| **Edit** | 既存ファイルの一部を書きかえる |
| **Bash** | ターミナルでコマンドを実行する（API呼び出し、スクリプト実行など） |
| **Glob** | ファイル名のパターンで検索する（例: `*.csv` で全CSVを探す） |
| **Grep** | ファイルの中身をキーワードで検索する |
| **WebFetch** | Webページの内容を取得する |

`tools:` で指定したものだけがそのエージェントに使える。書かなかったツールは使えない。
:::

quality-analystにはWriteもEditもない。**読んで分析するだけ、書き換えはしない。** この制限を入れた途端、出力の安定性が上がった。具体的にいうと:
...
全エージェントをOpusにすれば精度は上がるが、コストが跳ね上がる。Opusの推論コストはSonnetの約5倍。判断が必要な3体だけOpus、残り7体はSonnetにした。

:::details OpusとSonnetの違い
たとえるなら、**Opus = シニアコンサルタント、Sonnet = 優秀なアシスタント**。Opusは複雑な判断・戦略立案・曖昧な指示の解釈が得意だが、単価が高い。Sonnetはデータ集計・定型処理・フォーマット変換なら十分な精度で、高速かつ安い。全部シニアコンサルタントに頼む必要はない。
:::

- **Opus（判断系）：** qe-orchestrator、test-design-architect、extension-engineer
- **Sonnet（処理系）：** それ以外の7体

体感では、週次レポート1回の生成で数百円程度。全部Opusにすると千円を超える。判断が必要なフェーズだけOpusを使い、データ集計や整形はSonnetに任せるのがコスパの分岐点。

### 2. ツール権限の制限

「このエージェントは何を**しないか**」を決めるのが一番大事だった。具体的な失敗と対策は「エージェント定義ファイルの実物」セクションで後述する。
...
棚卸しした業務をグルーピングして、ロールに名前をつけた。「何をするか」より「何をしないか」を先に決めた方が上手くいった。

### 3. 定義ファイル作成

Markdownでfrontmatter + 本文を書く。ここで**擬人化**がポイント。「あなたはデータアナリストです」より「あなたは品質データを掘り下げる探偵です」の方が、出力のトーンが安定する。

> 💡 なぜ擬人化が効くのか？ AIは「データアナリスト」のような汎用的な役割だと、何をどこまでやるか迷って出力がブレる。「探偵」のように具体的なキャラクターを与えると、「深掘りする」「根拠を示す」「推理する」という行動パターンが絞られ、毎回似た構造の出力を返すようになる。
...
| 作業 | Before（1人で全部） | After（仮想QAチーム） |
|---|---|---|
| 週次レポート | 2〜3時間かけて手動集計 | 15分（指示＋確認のみ） |
| Qase棚卸し（217,948件） | 手をつけられなかった | 自動バッチで3分類 |
...
⏳ qe-orchestrator: 週次レポートですね。data-analyst → dashboard-designer
   → insight-reporter の順で進めます。

⏳ data-analyst: Qase APIから今週のテスト実行データを取得中...
   → 実行件数: 342件、Pass率: 94.7%、新規バグ: 8件

⏳ dashboard-designer: 集計データを受け取りました。HTMLレポートを生成中...
   → quality-report.html を作成しました

⏳ insight-reporter: レポートに分析コメントを追加中...
   → 完成しました。前週比でPass率が2.1pt改善しています。
```
...
- **伝言ゲーム問題。** エージェント間の引き継ぎで情報が欠落することがある。たとえばdata-analystが出した集計結果をdashboard-designerに渡すとき、口頭指示（プロンプト経由）だと数値の丸め方やカラム名がずれる。JSONファイルに中間出力を書き出して次のエージェントに読ませると安定するが、その分手間は増える
- **稼働率の偏り。** 10体のうち日常的につかうのは3〜4体。vendor-managerとai-reverse-engineerは月に数回しか出番がない
- **定義ファイルのメンテコスト。** 業務が変わるたびに更新が必要。ドキュメントと同じで、放置すると腐る
- **コスト管理の可視化が未実装。** Opus3体を頻繁につかうとコストがかさむが、まだ定量的に把握できていない
...
いきなり10体は要らない。最初は1体でいい。

**おすすめの最初の1体：** 自分が一番時間を使っている「繰り返し作業」を1つ選んで任せる。

たとえば、毎週やっている集計作業があるなら：
...
あなたは週次データの集計担当です。
指定されたフォルダのCSVファイルを読み込み、
以下の項目を集計してテーブル形式で報告してください。

- 今週の実行件数
- Pass/Fail/Blocked の内訳
- 前週比の増減
```

この1体で「AIに仕事を任せる感覚」を掴んでから、2体目を考えればいい。2体目は、1体目の出力を受け取って次の処理をするエージェント（レポート整形、可視化など）を作ると、パイプラインの威力が実感できる。
...
じつはこの記事を書いている時点で、エージェントは10体から35体に増えている。10体で固めた「分解して名前をつける」原則がそのままスケールした結果だが、その話はまた別の機会に。

もしあなたも「1人で何役もこなしている」なら、まず自分の業務を全部書き出すところから始めてみてほしい。自分の場合は10種類を超えたとき、エージェント化の余地が見えた。

[^qase]: クラウドベースのテスト管理プラットフォーム。テストケースの管理、テスト実行の記録、レポート生成などの機能を持つ。https://qase.io
[^opus-sonnet]: Claude（Anthropic社のAI）のモデルティア。Opusはもっとも推論力が高く複雑な判断に向くが高コスト。Sonnetはバランス型で高速・低コスト。
