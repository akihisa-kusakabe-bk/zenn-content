...
:::message
**この記事で使うもの**
- **Claude Code** — Anthropic社が提供するターミナル上で動くAIコーディングアシスタント。チャット形式でファイル操作・コード生成・分析などができる
...
quality-analystにはWriteもEditもない。**読んで分析するだけ、書き換えはしない。** この制限を入れた途端、出力の安定性が上がった。具体的に言うと:
...
実はこの記事を書いている時点で、エージェントは10体から35体に増えている。10体で固めた「分解して名前をつける」原則がそのままスケールした結果だが、その話はまた別の機会に。
